...
<style>
    ...
    [class*="st-key-main_page_nav"] label:has(input:checked) {
        ...
        box-shadow: 0 1px 3px rgba(61, 57, 41, 0.10) !important;
    }
    ...
    [class*="st-key-workbench_panel"] h3 {
        margin-top: 0 !important;
    }
    .page-nav-spacer {
        height: 0.45rem;
...
with workbench_content_col:
    ...
    if last_run is not None:
        ...
        image.thumbnail((MAX_INFERENCE_SIZE, MAX_INFERENCE_SIZE))
        st.info("当前显示的是预览图片。可以在左侧工作面板上传自己的图片，调整参数后点击“开始计算 / 更新结果”。")
